...
class Generator_Adain_Upsample_student_small(nn.Module):
    def __init__(self, input_nc, output_nc, latent_size, n_blocks=6, deep=False,
                 norm_layer=nn.BatchNorm2d,
                 padding_type='reflect'):
        assert (n_blocks >= 0)
        super(Generator_Adain_Upsample_student_small, self).__init__()

        activation = nn.ReLU(True)

        self.deep = deep

        self.first_layer = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(input_nc, 16, kernel_size=7, padding=0),
                                         norm_layer(16), activation)
        ### downsample
        self.down1 = nn.Sequential(nn.Conv2d(16, 32, kernel_size=3, stride=2, padding=1),
                                   norm_layer(32), activation)
        self.down2 = nn.Sequential(nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1),
                                   norm_layer(64), activation)
        ...
        for i in range(n_blocks):
            BN += [
                ResnetBlock_Adain(128, latent_size=latent_size, padding_type=padding_type, activation=activation)]
        self.BottleNeck = nn.Sequential(*BN)
        ...
        self.last_layer = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(16, output_nc, kernel_size=7, padding=0))
...
if __name__ == "__main__":
    ...
    net = Generator_Adain_Upsample_student_small(input_nc=3, output_nc=3, latent_size=512, n_blocks=18,deep=True)
    # flops, params = thop.profile(net, inputs=(img,latent), verbose=False)
    # print('#Params=%.2fM, GFLOPS=%.2f' % (params / 1e6, flops / 1e9))
    with torch.no_grad():
        macs = profile_macs(net, (img,latent))
        print(' MACs: %.2fG' % ( macs / 1e9), flush=True)
    ...
    #
    # with torch.no_grad():
    #     macs = profile_macs(net, (latent, img, None, None))
    #     print(' MACs: %.3fG' % ( macs / 1e9), flush=True)